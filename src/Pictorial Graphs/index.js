...
export function createPictorialChart(config = {}) {
  const {
    containerId = 'container',
    chartOptions = {},
    shapes = {},
    xAxisCategories = ['Type'],
    yAxisLabel = '{value} ft',
  ...
  const defaultConfig = {
    ...
    yAxis: { labels: { format: yAxisLabel } },
    legend: { enabled: true },
    series: []
  };
  
  // Merge with user options
  const finalConfig = { ...defaultConfig, ...chartOptions };
  
  // Create chart
  const chart = Highcharts.chart(containerId, finalConfig);
  
  // Return API
  return {
    addShape: (type, customData) => {
      if (activeShapes[type]) return false;
      
      const shapeData = customData || shapes[type];
      if (!shapeData) {
      ...
      activeShapes[type] = true;
      console.log(`Added shape: ${shapeData.name}`);
      return true;
    },
    
    removeShape: (type) => {
      const series = chart.series.find(s => s.userOptions.key === type);
      if (!series) return false;
      
      series.remove();
      delete activeShapes[type];
      console.log(`Removed shape: ${type}`);
      return true;
    },
    
    toggleShape: (type, customData) => {
      return activeShapes[type] 
        ? this.removeShape(type)
        : this.addShape(type, customData);
    },
    
    getChart: () => chart,
    getActiveShapes: () => Object.keys(activeShapes),
    
    // New: Add custom shape on the fly
    addCustomShape: (id, name, value, color, path) => {
      return this.addShape(id, {
        name: name,
        value: value,
        color: color,
        paths: { definition: path, max: value * 1.2 }
      });
    }
  };
}
